...
# attribute from the vcard to store it in a JSON format for front-end access.
def parsed_vcf(request):
    ...
    for v_card_content in file_content:
        ...
        photo_data = vcard.photo.value if hasattr(vcard, 'photo') else None
        
        fn_list = store_full_name_as_list(fn)
        # tel.append(extract_and_save_image(photo_data, fn_list[0].lower()))
        
        vcf_dict = {
            'fn' : fn_list,
            'photo' : extract_and_save_image(photo_data, fn_list[0].lower(), config('PHOTO_FOLDER_PATH')),
            'email' : store_emails(emails),
            'phone_no' : store_telephone_nos(tel_nos),
            'company' : vcard.org.value if hasattr(vcard, 'org') else ''
        }
        vcf_list.append( vcf_dict )

    return JsonResponse(vcf_list, safe=False)  # safe=False is needed if we're returning a list