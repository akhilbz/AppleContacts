  ...
  return (
    <>
      <ul className="text-[#9B9B9B] cursor-pointer">
        <li><button disabled={lists.length == 0 || newContactInstance} onClick={() => dispatch(setNewContactInstance(true))} 
        className={`flex w-full h-fit rounded-lg ${(lists.length != 0 && !newContactInstance) ? "hover:bg-[#007aff] hover:text-[#d4d4d4]" : ""}  px-2 py-1`}>New Contact</button></li>
        <li><button disabled={(lists.length == 0 || contactsLength == 0 || contactInfo == null || newContactInstance)} 
        className={`flex w-full h-fit rounded-lg ${(lists.length != 0 && contactsLength != 0 && contactInfo != null && !newContactInstance) ? 
